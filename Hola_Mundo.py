print("PROGRAMA PAR MI KOLALOKA \n\n (Tienes que escribir lo que se te pide y en cuanto acabes presionas enter)")
t=input ("Pon nombre a una tarea:   ")
n=input ("Ahora dime para quien es esta tarea:   ")
p=input ("Ahora cuantos puntos va a tener esta tarea:   ")
if t=="" or n=="" or p=="" 

    print("Ok entonces esta tarea es para: ", n ," y tiene que hacer: ",t," la cual tiene un valor de: ",p)